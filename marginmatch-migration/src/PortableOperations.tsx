...
export function PortableOperations(){
  ...
  return <div className='min-h-screen bg-[#07111f] p-4 text-white'><div className='mx-auto max-w-6xl'>
    ...
    <section className='mt-4 grid gap-4 lg:grid-cols-2'>
      ...
      <div className='rounded-3xl border border-slate-800 bg-slate-900 p-5'><h2 className='text-xl font-black'>Open exceptions</h2><div className='mt-4 space-y-2'>{data.recentExceptions.map((x,i)=><div key={String(x.id||i)} className='rounded-xl bg-slate-950 p-3'><div className='font-black'>{String(x.type||'Exception')}</div><div className='mt-1 text-xs text-slate-400'>{String(x.aiDecision?.recommendedAction||x.action||x.note||'Review')}</div></div>)}{!data.recentExceptions.length&&<div className='text-sm text-emerald-300'>No open exceptions.</div>}</div></div>
    </section>
